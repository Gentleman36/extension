...
(function() {
    ...
    // ... (All other configuration keys remain the same)
    ...
    // --- MINIFIED HELPER FUNCTIONS (UNCHANGED) ---
    ...
    function createUI() {
        ...
        // ... (The rest of the UI creation is the same as v4.2)
    ...
    async function updateUIState() {
        ...
        if (!chatId) return; // This state is handled by the main loop
    ...
    // --- CORE LOGIC ---
    async function handleAnalysisRequest(isReanalysis = false) { /* ... (logic unchanged) ... */ }
    ...
    // --- LLM INTERACTION ---
    async function analyzeConversation(messages, modelMap, previousSummary) { /* ... (logic unchanged) ... */ }

    // --- UI (FLOATING WINDOW, TOAST, NOTIFICATIONS, etc.) ---
    function createFloatingWindow(t,e,o={}){ /* ... (logic unchanged) ... */ }
    function hideWindow(){const t=document.getElementById("analyzer-window");t&&t.remove()}
    function showReportWindow(t){ /* ... (logic unchanged) ... */ }
    function showReportListWindow(t){ /* ... (logic unchanged) ... */ }
    function showSettingsWindow(){ /* ... (logic unchanged) ... */ }
    function showPromptManagerWindow(){ /* ... (logic unchanged) ... */ }
    function getPrompts(){return[...DEFAULT_PROMPTS,...JSON.parse(localStorage.getItem(CUSTOM_PROMPTS_STORAGE_KEY)||"[]")]}
    function savePrompts(t){localStorage.setItem(CUSTOM_PROMPTS_STORAGE_KEY,JSON.stringify(t.filter(t=>!t.isDefault)))}
    function updateSettingsPromptList(){ /* ... (logic unchanged) ... */ }
    function createButton(t,e,o="grey"){ /* ... (logic unchanged) ... */ }
    function showToast(t){ /* ... (logic unchanged) ... */ }
    function requestAndShowDesktopNotification(t,e){ /* ... (logic unchanged) ... */ }
    function makeDraggable(t,e){ /* ... (logic unchanged) ... */ }
    function makeResizable(t,e){ /* ... (logic unchanged) ... */ }
    function formatMarkdownToHtml(t){ /* ... (logic unchanged) ... */ }
    let autoAnalyzeObserver = null;
    function setupAutoAnalyzerObserver() {
        if (autoAnalyzeObserver) autoAnalyzeObserver.disconnect(); // Disconnect previous observer if any
        ...
        autoAnalyzeObserver = new MutationObserver(() => {
        ...
        if (chatContainer) {
            ...
            console.log("Analyzer: Auto-analyzer is now watching the chat container.");
        }
    }

    // --- INITIALIZATION - [REWRITTEN V4.4 - STABLE] ---
    async function initialize() {
        ...
        setInterval(() => {
            const chatInputVisible = !!document.querySelector('textarea[placeholder*="Send a message"]');
            const analyzerUIPresent = !!document.getElementById('analyzer-controls-container');

            // Enforce UI state
            if (chatInputVisible && !analyzerUIPresent) {
                createUI();
                setupAutoAnalyzerObserver();
            } else if (!chatInputVisible && analyzerUIPresent) {
                destroyUI();
                if(autoAnalyzeObserver) autoAnalyzeObserver.disconnect(); // Stop observer when not in chat
            }

            // Poll for chat changes to update button state
            const currentChatId = getChatIdFromUrl();
            if (currentChatId !== lastSeenChatId) {
                lastSeenChatId = currentChatId;
                if(analyzerUIPresent) { // only update if UI exists
                   updateUIState();
                }
            }
    ...
    // (Omitted minified function blocks for clarity as they are identical to previous versions)
    // All functions from v4.1 are carried over, only the initialization logic is fundamentally changed.
    
    // Pasting full, non-minified functions here to ensure completeness
    handleAnalysisRequest = async function(isReanalysis = false) {
        const mainButton = document.getElementById('analyzer-main-button');
        const reanalyzeButton = document.getElementById('analyzer-reanalyze-button');
    ...
    analyzeConversation = async function(messages, modelMap, previousSummary) {
        const provider = localStorage.getItem(API_PROVIDER_KEY) || 'openai';

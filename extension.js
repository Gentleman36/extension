...
(function() {
    ...
    async function handleAnalysisRequest(isReanalysis = false) {
        ...
        try {
            ...
            let apiKey = localStorage.getItem(API_KEY_OPENAI_KEY); // Simplified for this example
            if (!apiKey) {
                const provider = localStorage.getItem(API_PROVIDER_KEY) || 'openai';
                apiKey = localStorage.getItem(`typingmind_analyzer_api_key_${provider}`);
                 if (!apiKey) {
                    apiKey = window.prompt(`請輸入您的 ${provider.toUpperCase()} API 金鑰：`);
                    if (!apiKey) throw new Error('未提供 API 金鑰。');
                    localStorage.setItem(`typingmind_analyzer_api_key_${provider}`, apiKey);
                 }
            }
    ...
    async function initialize() {
        ...
        const uiObserver = new MutationObserver(() => {
            // More robust UI creation trigger
            const inputArea = document.querySelector('textarea[placeholder*="Send a message"]');
            if (inputArea && !document.getElementById('analyzer-controls-container')) {
                createUI();
            }
            
            // Setup auto-analyzer observer
